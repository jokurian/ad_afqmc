...
def FD_integrals_chol(mf,dR= 1e-5,chol_cut=1e-5):
    ...
    for i in range(len(atom_symbols)):
        ...
        for j in [2]:#range(3):
            ...
            basis_p = np.array(fractional_matrix_power(mf_p.get_ovlp(), -0.5),dtype="float64")
            ...
            #mf_m.kernel()
            basis_m = np.array(fractional_matrix_power(mf_m.get_ovlp(), -0.5),dtype="float64")
            ...
            h2_der = pyscf_interface.modified_cholesky(deri.reshape(mol.nao**2,mol.nao**2),max_error = chol_cut)
            import pdb;pdb.set_trace()
            h1_der = (h1_p - h1_m)/(2.*dR)
    ...
    np.savez("Integral_der.npz",array1 = h1_der_array,array2=h2_der_array,array3=h0_der,dm=dm0)    
...
def FD_integrals(mf,dR=0.00001):
    ...
    if(isinstance(mf,scf.uhf.UHF)):
        dm0 = [basis0.T @ mf.get_ovlp() @ mf.make_rdm1()[0] @ mf.get_ovlp() @ basis0,basis0.T @ mf.get_ovlp() @ mf.make_rdm1()[1] @ mf.get_ovlp() @ basis0]
...
def write_integrals_lowdins(mf):
    ...
    if(isinstance(mf,scf.uhf.UHF)):
        ...
        np.savez("rhf.npz", mo_coeff=q)#np.savez("rhf.npz", wave_data=q,mo_coeff=[basis0,basis0])
...
def blocking_nuc_grad(npz_filename='grad_afqmc.npz',neql=0, printQ=False, writeBlockedQ=False,printG=True):
    ...
    for i in range(natm):
        for j in range(3):
            ...
            if(np.sum(mask == False)):
                print(f"({i},{j}) Outliers removed: {np.sum(mask == False)}");# print(gradients[:,i,j][~mask])
            grad_avg, grad_err = stat_utils.blocking_analysis(weights[mask],data,neql=neql,printQ=printQ,writeBlockedQ=writeBlockedQ)
            if(grad_avg is None): print(f"({i},{j}) Couldnt find average estimate");grad_avg = 0.0
            if(grad_err is None):
                if(grad_avg == 0.0):
                    grad_err = 0.0
                else:
                    print(f"({i},{j}) Couldnt find error estimate");grad_err = 0.0
            avg[i,j] = grad_avg
            error[i,j] = grad_err
...
def get_rdmsDer_uhf(norb,nchol,filename="en_der_afqmc.npz",nproc = 4):
    ...
    for i in range(nproc):
        weighti = np.load(f"en_der_afqmc_{i}.npz")["weight"]
        #import pdb;pdb.set_trace()
        rdm1i = np.load(f"en_der_afqmc_{i}.npz")["rdm1"].reshape(2,weighti.shape[0],norb,norb)
        rdm2i = np.load(f"en_der_afqmc_{i}.npz")["rdm2"].reshape(weighti.shape[0],nchol,norb,norb)
...
def calculate_nuc_gradients(integral_der = "Integral_der.npz", energy_der= "en_der_afqmc.npz",printG=True,printQ=False,uhf=False):
    ...
    if(uhf):
        rdm1,rdm2,weights = get_rdmsDer_uhf(h1_der.shape[2],h2_der.shape[2],nproc= find_nproc())
        obs1 = np.einsum("rxpq,npq->nrx",h1_der,rdm1[:,0,:,:]) + np.einsum("rxpq,npq->nrx",h1_der,rdm1[:,1,:,:])
        obs2 = np.einsum("rxgpq,ngpq->nrx",h2_der,rdm2) #[0]) + np.einsum("rxgpq,ngpq->nrx",h2_der,rdm2[1])
        #import pdb;pdb.set_trace()
        obs0 = h0_der
        gradients = obs1 + obs2 + obs0
    else: 
        rdm1,rdm2,weights = get_rdmsDer_rhf(h1_der.shape[2],h2_der.shape[2],nproc= find_nproc())
    ...
    for i in range(natm):
        for j in range(3):
            ...
            if(np.sum(mask == False)):
                print(f"({i},{j}) Outliers removed: {np.sum(mask == False)}");# print(gradients[:,i,j][~mask])
            grad_avg, grad_err = stat_utils.blocking_analysis(weights[mask],data,neql=0,printQ=printQ,writeBlockedQ=False)
            #import pdb;pdb.set_trace()
